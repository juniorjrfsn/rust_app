...
impl Neuron {
    ...
    fn activate(&self, inputs: &[f64], activation_fn: &str) -> f64 {
        ...
        match activation_fn {
            "relu" => sum.max(0.0), // ReLU activation
            ...
            for (inputs, targets) in training_data.iter() {
                ...
                for l in (0..self.layers.len() - 1).rev() {
                    ...
                    for j in 0..self.layers[l].len() {
                        ...
                        layer_deltas.push(delta_sum * if activation > 0.0 { 1.0 } else { 0.0 }); // ReLU derivative
                    }
...
fn main() {
    ...
    mlp.train(&training_data, 0.2, 20000); // Aumente a taxa de aprendizado e o número de épocas
