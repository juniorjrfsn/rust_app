    ...
    fn train(&mut self, training_data: &[(Vec<f64>, Vec<f64>)], learning_rate: f64, epochs: usize) {
        for epoch in 0..epochs {
            ...
            for (inputs, targets) in training_data.iter() {
                ...
                // Backpropagation
                let mut deltas = vec![output_errors.iter().zip(output.iter()).map(|(err, out)| err * out * (1.0 - out)).collect::<Vec<f64>>()]; // Output layer deltas
...
fn main() {
    ...
    mlp.train(&training_data, 0.5, 10000);
