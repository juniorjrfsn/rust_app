...
fn main() {
    ...
    // Create training data *outside* the main vector
    ...
    let training_data: Vec<(&[f64], i32)> = vec![
        (&all_zeros, 0),       // Should be 0
        (&all_ones, 1),        // Should be 1
        (&half_gray, 1),       // Should be 1 (closer to 1)
        (&dark_gray, 0),       // Should be 0
        (&light_gray, 1),      // Should be 1
        (&almost_all_ones, 1), // Should be 1
        (&almost_all_zeros,0), // Should be 0
    ];
    ...
    for epoch in 0..epochs {
        ...
        if epoch % 10 == 0 {
            ...
            println!("Accuracy: {}/{} = {}", correct_predictions, training_data.len(), correct_predictions as f32 / training_data.len() as f32);
        }
    }

    // Test data (also create test data outside the prediction call)
    ...
    println!("Prediction for all zeros: {}", perceptron.predict(&test_all_zeros)); // Should be 0
    println!("Prediction for all ones: {}", perceptron.predict(&test_all_ones));   // Should be 1
    println!("Prediction for half gray: {}", perceptron.predict(&test_half_gray)); // Should be 1
    println!("Prediction for quarter gray: {}", perceptron.predict(&test_quarter_gray)); //Should be 0
