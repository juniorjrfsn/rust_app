...
use std::fs::{self, File}; // Importação corrigida
use std::io::{self, Write, Read};
use image::{ImageBuffer, Rgb, RgbImage};
use imageproc::drawing::draw_text_mut;  // Importação corrigida
...
impl CNN {
    ...
    fn train(&mut self, image: &Array2<f64>, target: &Array1<f64>) {
        ...
        // Backprop through pooling and convolution
...
impl TextClassifier {
    fn save(&self, path: &str) -> io::Result<()> {
        ...
        let mut file = File::create(path)?; // Uso corrigido
        file.write_all(serialized.as_bytes())?;
        Ok(())
    }

    fn load(path: &str) -> io::Result<Self> {
        let mut file = File::open(path)?; // Uso corrigido
...
fn generate_char_image(ch: char, size: u32, font_path: &str) -> RgbImage {
    ...
    // Certifique-se de que o arquivo de fonte existe
    if !fs::metadata(font_path).is_ok() {
        panic!("Font file not found at {}", font_path);
    }

    // Carregue a fonte usando FontRef
    let font_data = fs::read(font_path).expect("Failed to read font file");
    let font = FontRef::try_from_slice(&font_data).expect("Failed to load font");

    // Defina a escala da fonte
    let scale = AbScale {
        x: size as f32 * 0.7,
        y: size as f32 * 0.7,
    };

    // Desenhe o caractere na imagem
    draw_text_mut(
        &mut img,
        Rgb([0, 0, 0]), // Cor do texto (preto)
        (size / 4) as i32,
        (size / 4) as i32,
        scale,
        &font,
...
fn train_model() -> io::Result<()> {
    ...
    // Listar todas as fontes no diretório C:\Windows\Fonts
    ...
    let mut classifier = TextClassifier {
        cnn: CNN::new(8, 5, char_classes.len()),
